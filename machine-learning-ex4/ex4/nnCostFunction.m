...
% recode y to Y(% building the Y matrix of results)
I = eye(num_labels);
...
% feedforward(add ones to the A matrices)
A1 = [ones(m, 1) X];
Z2 = A1 * Theta1';
...
% calculte penalty(we need to remove the first column of each Theta first)
Jreg = lambda / (2 * m) * (sum(sum(Theta1(:, 2:end) .^2)) + sum(sum(Theta2(:, 2:end) .^2)));

% calculate J (reuse the cost function with matrix of results)
